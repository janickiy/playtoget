...
if($_SESSION['user_authorization'] == "ok"){
		...
		case 'create':
			...
			if(empty($error)){
				...
				if($id_sport_block){
					...
					if(Photoalbum::getNumberAlbums($id_sport_block, 'fitness') == 0){
						$fields = array();
						$fields['id'] = 0;
						$fields['name'] = $name;
						$fields['created_at'] = date("Y-m-d H:i:s");
						$fields['photoalbumable_type'] = 'fitness';
						$fields['id_owner'] = $id_sport_block;		
			
						Photoalbum::createAlbum($fields);	
					}				
				
					header('Location: http://' . $_SERVER['SERVER_NAME'] . '/?task=fitness&id_sport_block=' . $id_sport_block);		
					exit();
				}		
				else
	...
	if($_GET['id_sport_block']){	
		...
		if(SportBlocks::checkExistence($id_sport_block, 'fitness') or !is_numeric($_GET['id_sport_block'])){
		...
		$tpl->assign('SPORT_BLOCK_TYPE', 'fitness');
		$tpl->assign('ID_OWNER', $sport_block['id_owner']);
		$tpl->assign('ID_SPORT_BLOCK_PHOTO_ALBUM', SportBlocks::getSportBlockIdPhotoAlbum($id_sport_block, 'fitness'));
	
		if(Photoalbum::getNumberAlbums($id_sport_block, 'fitness') > 0){
	
			$arr_photos = Photoalbum::getAllPicList(SportBlocks::getSportBlockIdPhotoAlbum($id_sport_block, 'fitness'));
		...
		switch ($_GET['q']) {
			...
			case 'edit':
				...
				$tpl->assign('TITLE_PAGE', core::getLanguage('title', 'edit_fitness'));
				$tpl->assign('FORM_ACTION', 'edit');
				$tpl->assign('STR_NAME', core::getLanguage('str', 'fitness'));		
		...
		if($_GET['q'] == 'create'){
			$tpl->assign('QUERY', $_GET['q']);		
			$tpl->assign('TITLE_PAGE', core::getLanguage('title', 'create_fitness'));			
			$tpl->assign('ACTION', $_SERVER['REQUEST_URI']);		
			...
			$tpl->assign('BUTTON', core::getLanguage('button', 'create_fitness'));
		}
		else{		
			$tpl->assign('TITLE_PAGE', core::getLanguage('title', 'fitness'));
			...
			$arr_sportblocks = SportBlocks::getSportBlocks('fitness', 10, 0);
